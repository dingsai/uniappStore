<template>
	<view class="home">
		<view class="home_top" :style="{height: containTop +'px',paddingTop:top + 'px'}">
			<!-- 首页自定义顶部 -->
			<view class="titleBar" :style="{height:height+'px',lineHeight:height+'px',top: top +'px'}">
				首页
			</view>
		</view>
		<!-- 主体部分 -->
		<view class="main_contain" :style="{top:containTop +'px'}">
			<!-- 顶部搜索  轮播图-->
			<view class="top_contain">
				<view class="slider-bg"></view>
				<view class="search">
					<uni-icons class="bars_icon" type="bars" size="26" color="#ffffff" @tap="toCate"></uni-icons>
				...
				<view class="home_swiper">
					<swiper class="swiper" :indicator-dots="true" :autoplay="true" :interval="3000" :duration="800">
						<swiper-item v-for="item,i in swiperList" :key="i" @tap="toDetail(i)">
